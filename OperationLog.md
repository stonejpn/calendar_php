...
IntelliJから実行できるように設定する。



「プロジェクト設定」からSDKをPHPにしようとするとこの↓のエラーになる。

```
PHP インタープリター構成

PHPインタープリターはデフォルトSDKに指定できません。
現在のプロジェクトに対して「設定|PHP」で設定することができます。
```
...
https://pleiades.io/help/idea/remote-development-starting-page.html

これじゃなかった感。



`PHP WSL Support`プラグインが必要だった。

これで、WSL上のphpにアクセスできた。
...
確認すると、WSL2のAlmaLinux9のPHPは、8.0.30。

8.0ってEOLを迎えてるんだけど。。。(AlmaLinux側でセキュリティパッチは、バックポートされるみたい)
